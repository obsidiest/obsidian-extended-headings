...
- Fixed renaming H1–H12 headings referenced by nested heading paths, including links to descendants, wiki links, Markdown links, and embeds. Preserve unrelated path segments, explicit aliases, labels, titles, and allowed punctuation; distinguish repeated headings by their resolved source position.
- Routed the plugin rename command and supported native editor command/context-menu action through the same handler, retaining native command IDs/hotkeys and restoring the original handler on unload.
- Added Heading Hover Breadcrumb in the editor and default Outline across Live Preview, Source, and Reading mode, adapting Nested Properties Advanced 2.0.0. Includes full-marker/full-row activation, a scrollable ancestor hierarchy, current-heading highlighting, hover preview without moving the caret, click navigation, keyboard navigation, long-title wrapping, and fractional per-mode dismissal timers.
- Corrected breadcrumb SVG sizing that introduced unnecessary scrollbars and clipped short ancestor lists. The default 400 px maximum height accommodates twelve short titles; taller popovers use the available scroll area.
- Made editor marker activation cover the left margin and the full painted marker even when editor content overlaps its gutter. Popovers respect the dismissal timeout during scrolling and stay open across ancestor-preview layout updates and pointer travel through the connecting gap.
- Placed Heading Hover Breadcrumb last in main settings, named its timeout subsection Heading Hover Breadcrumb Popover Timeout, and defaulted the global Active Root-Level ⟺ Orphan Heading Threading option off. Existing saved choices are preserved.
- Added global and per-pane breadcrumb marker, static-guide, and threading controls with the requested parent dependencies and defaults. Includes regular, root-level, orphan, combined bidirectional, all-branches, and selected-heading threading submodes.
- Added shared and independently overridable per-pane Style Settings for breadcrumb decorations, plus popup appearance, geometry, typography, and main-heading highlights. Heading markers default to All small caps; numeric controls support precise values.
- Cached heading models per editor document and Reading render, and tied window/section/popover listeners and decorations to their lifecycle. Existing 1.0.1 Outline behavior is retained.
- Added rename, settings, DOM lifecycle, and optional real-browser CodeMirror/layout regression checks. Actual Windows Obsidian and mobile device validation remains separate.
