  ...
  for (const [label, matchingSpecs] of specsByLabel) {
    ...
    if (itemIndexes.length === matchingSpecs.length) {
      itemIndexes.forEach((itemIndex, index) => {
        matches.push({ itemIndex, svgMarkup: matchingSpecs[index].svgMarkup });
      });
      continue;
    }

    const first = matchingSpecs[0];
    const firstSignature = first ? JSON.stringify(first.svgMarkup) : "";
    if (
      first &&
      matchingSpecs.every((spec) => JSON.stringify(spec.svgMarkup) === firstSignature)
    ) {
      for (const itemIndex of itemIndexes) {
        matches.push({ itemIndex, svgMarkup: first.svgMarkup });
      }
    }
  }
...
export class CoreOutlineSvgRenderer {
  ...
  private async runPass(attachment: OutlineAttachment): Promise<void> {
    ...
    this.mutateWithoutObserving(attachment, () => {
      ...
      for (const match of matchOutlineSvgSpecs(labels, specs)) {
        ...
        if (item) this.appendSanitizedSvgs(item, match.svgMarkup);
      }
    });
  }
  ...
  private appendSanitizedSvgs(item: HTMLElement, markupItems: string[]): void {
    const wrapper = item.createSpan({
      cls: OUTLINE_SVG_CLASS,
